...
const Profile = () => {
    ...
    const { data: otherUser, error, mutate } = useSWR(
        `/api/user/${params.id}`,
        () =>
            axios
    ...
    const deleteProfile = (user, mutate, authMutator) => {
        axios.delete(`api/users/deleteProfilePhoto/${user}`).then(() => {
            mutate()
            authMutator()
        })
    }
        ...
        <>
            <section className="w-auto mx-auto bg-[#20354b] rounded-2xl px-8 py-6 shadow-lg">
                <div className="flex items-center justify-between">
                    <div>
                        <span
                            className={`text-white font-bold text-sm p-1 ${
                                !!otherUser?.is_admin
                        ...
                        <span
                            className={`ml-1 text-white font-bold text-sm p-1 ${
                                !!!otherUser?.is_admin
                                    ? 'bg-red-500 border-red-900'
                                    : 'bg-emerald-500 border-emerald-900'
                            } rounded-md border-2`}>
                    ...
                    {user?.id !== Number(params.id) && (
                        <div className="flex">
                            <Link href={`/direct/${otherUser?.id}`}>
                                <button className="bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-600 hover:to-indigo-600 text-white font-bold py-2 px-4 rounded-md flex items-center shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                                    <svg
                ...
                <div className="mt-6 w-fit mx-auto">
                    {otherUser?.avatar ? (
                        <img
                            ...
                            className="rounded-full fill-current h-28 w-28"
                            alt="profile picture"
                            srcSet=""
                        />
                    ) : (
                ...
                <p className="flex justify-center items-center -mt-0.5 text-emerald-400 text-sm text-center font-semibold mt-2.5">
                    ...
                    <span>
                        <svg
                            ...
                            viewBox="0 0 24 24"
                            fill="currentColor"
                            className="w-2 h-2 mx-1 text-white">
                            <path
                                fillRule="evenodd"
                ...
                {(user?.is_admin || user?.id === Number(params.id)) && (
                    <div className="flex justify-center mt-5 text-sm text-white">
                        ...
                        {otherUser?.address ??
                            'no address is inserted for this user.'}
                    </div>
                )}
            ...
            {isAdministratrOrCurrentUser(user, params) && formData !== null && (
                <div className="mt-3">
                    ...
                    <form
                        onSubmit={e => {
                            handleSubmit(e, params.id, formData, mutate)
                        }}
                        ...
                        <div className="mb-6">
                            ...
                            <textarea
                                ...
                                onChange={handleChange}
                                className="mt-1 p-2 block w-full rounded-md text-gray-900 border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
                        </div>
                        {!!user?.is_admin && (
                            <>
